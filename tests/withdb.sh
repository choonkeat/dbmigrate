...
case $DATABASE_DRIVER in
    ...
    until docker exec -it `cat cid.txt` pg_isready 2>&1 > /dev/null; do sleep 3; done
    ...
    echo "waiting for ${DATABASE_DRIVER}...";
    until docker exec -it `cat cid.txt` mysql -e 'SELECT 1' 2>&1 > /dev/null; do sleep 3; done
    sleep 20 # ready not ready
